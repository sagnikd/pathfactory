...
const FINAL_STATE_CHECK = [
  ...
  '- Output the JSON object and nothing else. No prose before it, no prose after it, no code fences. The visitor sees only what is inside "answer".',
  '- If the message you are about to send ends with a question, check whether that question has a natural closed set of short valid answers (industry, role, focus area, team size, timeline, budget band, yes/no). If it does, "suggestedQuestions" MUST list those answers — and if the flow above already names the options for that step, use exactly those. Sending a question like this with an empty "suggestedQuestions" is a mistake. Leave it empty ONLY when the question genuinely expects free text (name, email, company, phone) or an open opinion.',
]
